...
<body onload="myFunction()" style="margin:0;">
	...
	<div class="container text-center" style="padding-bottom: 70px;">
		<h4 class="animated bounceInRight" style="padding-bottom: 40px;">Edit product</h4>
	<form method='POST' enctype="multipart/form-data" name="izmeniA">
		<div class="form-group">
			<?php
				include("../includes/connection.php");

				$query = "select * FROM artikli where artikal_id='$_GET[aid]'";
				...
				echo '<div class="form-group">
								...
								<input type="text" required class="text-center" name="nazivartikla" style="width:500px;" value="'.$row['artikal_naziv'].'">
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Subcategory</label><br>
								<input type="text" required class="text-center" disabled style="width:500px;" value="'.$row['artikal_podkat'].'">
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Change subcategory</label><br>
								<select class="text-center" required name="kategorijaartikla" style="width:500px;">';
											include("../includes/connection.php");
											...
											{
												echo "<option selected value=\"No\">";
												echo "<option disabled>".$roww['kategorija_ime'];

												$q2 = "select * from podkat where glavnakat_id = ".$roww['kategorija_id'];
				...
				echo '<div class="form-group">
								...
								<textarea class="text-center" required name="cenaartikla" style="resize:none; width:500px; height:200px;">'.$row['artikal_opis'].'</textarea>
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Price</label><br>
								<input type="text" class="text-center" required name="cenaartikla" style="width:500px;" value="'.$row['artikal_cena'].'">
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Warranty</label><br>
								<input type="text" class="text-center" required name="garancijaartikla" style="width:500px;" value="'.$row['artikal_garancija'].'">
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">In stock</label><br>
								<input type="text" class="text-center" required name="kzaprodaju" style="width:500px;" value="'.$row['artikal_prodaja'].'">
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Image</label><br>
								<input type="file" class="text-center" required name="slikaartikla" size="35">
							</div>';
			?>
		</div>
  ...
  if(isset($_POST['dugme']))
  {
		...
		if(!(strtoupper(substr($_FILES['slikaartikla']['name'],-4))==".JPG" || strtoupper(substr($_FILES['slikaartikla']['name'],-5))==".JPEG"|| strtoupper(substr($_FILES['slikaartikla']['name'],-4))==".GIF" || strtoupper(substr($_FILES['slikaartikla']['name'],0))==""))
		{
		...
		else
		{
			...
			$A_SLIKA = "upload_image/".$_FILES['slikaartikla']['name'];

			$A_NAZIV=$_POST['nazivartikla'];
			$A_KATEGORIJA=$_POST['kategorijaartikla'];
			$A_OPIS=$_POST['opisartikla'];
			$A_KZAPRODAJU=$_POST['kzaprodaju'];
			$A_GARANCIJA=$_POST['garancijaartikla'];
			$A_CENA=$_POST['cenaartikla'];

			$updateartikal=$_GET['aid'];

			if($_FILES['slikaartikla']['name'] == NULL)
			{
				if(isset($A_KATEGORIJA) == "No")
				{
					$queryq = "update artikli SET artikal_naziv=?, artikal_opis=?, artikal_cena=?, artikal_garancija=?, artikal_prodaja=? WHERE artikal_id=?";
					$stmt= $conn->prepare($queryq);
					$stmt->execute([$A_NAZIV, $A_OPIS, $A_CENA, $A_GARANCIJA, $A_KZAPRODAJU, $updateartikal]);
				}
				else
				{
					$queryq = "update artikli SET artikal_naziv=?, artikal_podkat=?, artikal_opis=?, artikal_cena=?, artikal_garancija=?, artikal_prodaja=? WHERE artikal_id=?";
					$stmt= $conn->prepare($queryq);
					$stmt->execute([$A_NAZIV, $A_KATEGORIJA, $A_OPIS, $A_CENA, $A_GARANCIJA, $A_KZAPRODAJU, $updateartikal]);
				}
				$_SESSION['ps'] = "You have successfully edited the product.";
				header("Location: artikli.php");
			}
			else
			{
				if(isset($A_KATEGORIJA) == "No")
				{
					$query = "update artikli SET artikal_naziv=?, artikal_opis=?, artikal_cena=?, artikal_garancija=?, artikal_prodaja=?, artikal_slika=? WHERE artikal_id=?";
					$stmt= $conn->prepare($query);
					$stmt->execute([$A_NAZIV, $A_OPIS, $A_CENA, $A_GARANCIJA, $A_KZAPRODAJU, $A_SLIKA, $updateartikal]);
				}
				else
				{
					$query = "update artikli SET artikal_naziv=?, artikal_podkat=?, artikal_opis=?, artikal_cena=?, artikal_garancija=?, artikal_prodaja=?, artikal_slika=? WHERE artikal_id=?";
					$stmt= $conn->prepare($query);
					$stmt->execute([$A_NAZIV, $A_KATEGORIJA, $A_OPIS, $A_CENA, $A_GARANCIJA, $A_KZAPRODAJU, $A_SLIKA, $updateartikal]);
				}
				$_SESSION['ps'] = "You have successfully edited the product.";
				header("Location: artikli.php");
			}
