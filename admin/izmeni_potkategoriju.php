...
<body onload="myFunction()" style="margin:0;">
	...
	<div class="container text-center" style="padding-bottom: 70px;">
		<h4 class="animated bounceInRight" style="padding-bottom: 40px;">Edit subcategory</h4>
	<form method='POST' enctype="multipart/form-data" name="izmeniPK">
		<div class="form-group">
			<?php
				include("../includes/connection.php");

				$query = "select * FROM podkat where podkat_id='$_GET[pid]'";
				...
				echo '<div class="form-group">
								...
								<input type="text" required class="text-center" name="nazivpk" style="width:500px;" value="'.$row['podkat_naziv'].'">
							</div>';
              include("../includes/connection.php");

              $query1="select * from kategorija where kategorija_id=".$row['glavnakat_id']."";
              ...
              $rows = $stmt->fetch();
				...
				echo '<div class="form-group">
								...
								<input type="text" required class="text-center" disabled style="width:500px;" value="'.$rows['kategorija_ime'].'">
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Change category</label><br>
                <select class="text-center" required name="kat" style="width:500px;">';
											include("../includes/connection.php");

											$queryy="select * from kategorija";
                      ...
                      echo "<option selected value=\"No\">";
											...
											{
												echo '<option value="'.$roww['kategorija_id'].'">'.$roww['kategorija_ime'];
											}
								echo'</select>
							</div>';
				echo '<div class="form-group">
								<label for="exampleFormControlTextarea1">Image</label><br>
								<input type="file" class="text-center" name="slikapotkategorije" size="35">
							</div>';
			?>
		</div>
  ...
  if(isset($_POST['dugme']))
  {
		...
		if(!(strtoupper(substr($_FILES['slikapotkategorije']['name'],-4))==".JPG" || strtoupper(substr($_FILES['slikapotkategorije']['name'],-5))==".JPEG"|| strtoupper(substr($_FILES['slikapotkategorije']['name'],-4))==".GIF" || strtoupper(substr($_FILES['slikapotkategorije']['name'],0))==""))
		{
		...
		{
			move_uploaded_file($_FILES['slikapotkategorije']['tmp_name'],"../upload_image/".$_FILES['slikapotkategorije']['name']);
			$PK_SLIKA = "upload_image/".$_FILES['slikapotkategorije']['name'];

			$PK_NAZIV=$_POST['nazivpk'];
			$PK_KATEGORIJA=$_POST['kat'];

			$PID=$_GET['pid'];

			if($_FILES['slikapotkategorije']['name'] == NULL)
			{
				if($PK_KATEGORIJA == "No")
				{
					$queryq = "update podkat SET podkat_naziv=? WHERE podkat_id=?";
					$stmt= $conn->prepare($queryq);
					$stmt->execute([$PK_NAZIV, $PID]);
				}
				else
				{
					$queryq = "update podkat SET glavnakat_id=?, podkat_naziv=? WHERE podkat_id=?";
					$stmt= $conn->prepare($queryq);
					$stmt->execute([$PK_KATEGORIJA, $PK_NAZIV, $PID]);
				}
				$_SESSION['ps'] = "You have successfully edited the subcategory.";
				header("Location: potkategorije.php");
			}
			else
			{
				if($PK_KATEGORIJA == "No")
				{
          $queryq = "update podkat SET podkat_naziv=?, podkat_slika=? WHERE podkat_id=?";
					$stmt= $conn->prepare($queryq);
					$stmt->execute([$PK_NAZIV, $PK_SLIKA, $PID]);
				}
				else
				{
          $queryq = "update podkat SET glavnakat_id=?, podkat_naziv=?, podkat_slika=? WHERE podkat_id=?";
					$stmt= $conn->prepare($queryq);
					$stmt->execute([$PK_KATEGORIJA, $PK_NAZIV, $PK_SLIKA, $PID]);
				}
