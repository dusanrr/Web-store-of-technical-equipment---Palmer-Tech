...
<body onload="myFunction()" style="margin:0;">
	...
	<div class="container text-center" style="padding-bottom: 70px;">
		...
		<tbody>
			<?php
				...
				$totalEmpSQL = "select count(*) as broj FROM kategorija";
				...
				$empSQL = "select * FROM kategorija order by kategorija_id ASC LIMIT $startFrom, $showRecordPerPage";
				...
				{
					echo "<tr>";
					echo "<td>".$row['kategorija_id'];
					echo "<td>".$row['kategorija_ime'];
					echo '<td><img src="../'.$row['kategorija_slika'].'" class="lele" style="width:156px;height:156px"></td>';
					echo '<td><a class="btn btn-danger" href="obrisi.php?kid='.$row['kategorija_id'].'"><i class="fas fa-trash"></i></a>';
					echo '<td><a class="btn btn-success" href="izmeni_kategoriju.php?kid='.$row['kategorija_id'].'"><i class="fas fa-edit"></i></a>';
					echo '<td><a class="btn btn-info" href="" data-toggle="modal" data-target="#addnc"><i class="fas fa-plus"></i></a>';
					echo "</tr>";
				}
			?>
		</tbody>
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header bg-dark text-white d-inline-block">
					...
					<h5 class="modal-title text-center font-weight-bold" style="" id="exampleModalLongTitle">Create new category</h5>
				</div>
				<div class="modal-body justify-content-md-center" align="center">
					<form method='POST' enctype="multipart/form-data" name="dodajkategoriju">
						<div class="form-group">
							<label for="recipient-name" class="col-form-label font-weight-bold">Category name</label>
							<input type="text" class="form-control" required id="recipient-name" name="cat">
						</div>
						<div class="form-group">
							<label for="recipient-name" class="col-form-label font-weight-bold">Image</label>
							<input type="file" class="form-control" required name="slikak">
						</div>
				</div>
				<div class="modal-footer bg-dark text-white justify-content-md-center">
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
					<input type="submit" name="DK" value="Create" class="btn btn-info"/>
					</form>
				</div>
			</div>
		</div>
	...
	if(isset($_POST['DK']))
	{
		...
		if(file_exists("../upload_image/".$_FILES['slikak']['name']))
		{
			$_SESSION['perr'] = "File already uploaded. Please do not updated with same name.";
		}
		move_uploaded_file($_FILES['slikak']['tmp_name'],"../upload_image/".$_FILES['slikak']['name']);
		$KATEGORIJA_SLIKA = "upload_image/".$_FILES['slikak']['name'];

		$cat=$_POST['cat'];

		$query="insert into kategorija(kategorija_ime, kategorija_slika) values('$cat', '$KATEGORIJA_SLIKA')";

		$conn->exec($query);
		$_SESSION['ps'] = "You have successfully added a new category.";
