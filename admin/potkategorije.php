...
<body onload="myFunction()" style="margin:0;">
	...
	<div class="container text-center" style="padding-bottom: 70px;">
		<h4 class="animated bounceInRight text-center" style="padding-bottom: 10px;">ORDERS</h4>
		...
		<thead class="text-white" style="background: #1F2833; opacity: 0.8;">
			<tr>
				...
				<th scope="col">Edit subcategory</th>
		...
		<tbody>
			<?php
				...
				$totalEmpSQL = "select count(*) as broj FROM podkat";
				...
				$empSQL = "select * FROM podkat P inner join kategorija K where P.glavnakat_id=K.kategorija_id order by podkat_id ASC LIMIT $startFrom, $showRecordPerPage";
				...
				{
					echo "<tr>";
					echo "<td>".$row['podkat_id'];
					echo "<td>".$row['kategorija_ime']." (ID: ".$row['kategorija_id'].")";
					echo "<td>".$row['podkat_naziv'];
					echo '<td><img src="../'.$row['podkat_slika'].'" class="lele" style="width:156px;height:156px"></td>';
					echo '<td><a class="btn btn-danger" href="obrisi.php?pid='.$row['podkat_id'].'"><i class="fas fa-trash"></i></a>';
					echo '<td><a class="btn btn-success" href="izmeni_potkategoriju.php?pid='.$row['podkat_id'].'"><i class="fas fa-edit"></i></a>';
					echo '<td><a class="btn btn-info" href="" data-toggle="modal" data-target="#addnsc"><i class="fas fa-plus"></i></a>';
					echo "</tr>";
				}
			?>
		</tbody>
	...
	<div class="modal fade" id="addnsc" tabindex="-1" role="dialog" aria-labelledby="addnc" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header bg-dark text-white d-inline-block">
					...
					<h5 class="modal-title text-center font-weight-bold" style="" id="exampleModalLongTitle">Create new subcategory</h5>
				</div>
				<div class="modal-body justify-content-md-center" align="center">
					<form method='POST' enctype="multipart/form-data" name="dodajpotkategoriju">
						<div class="form-group">
							<label for="recipient-name" class="col-form-label font-weight-bold">Select category</label>
							<select class="form-control" required name="glavna">
								<?php
									include("../includes/connection.php");
									...
									while($row=$stmt->fetch())
									{
										echo "<option value='".$row['kategorija_id']."'>".$row['kategorija_ime'];
										//PASS ID NOT NAME
									}
									$conn = null;
								?>
						...
						<div class="form-group">
							<label for="recipient-name" class="col-form-label font-weight-bold">Subcategory name</label>
							<input type="text" class="form-control" required id="recipient-name" name="podkat">
						</div>
						<div class="form-group">
							<label for="recipient-name" class="col-form-label font-weight-bold">Image</label>
							<input type="file" class="form-control" required name="slikapk">
						</div>
				</div>
				<div class="modal-footer bg-dark text-white justify-content-md-center">
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
					<input type="submit" name="dpk" value="Create" class="btn btn-info"/>
					</form>
				</div>
			</div>
		</div>
	...
	<?php
			if(isset($_POST['dpk']))
			{
				...
				if(file_exists("../upload_image/".$_FILES['slikapk']['name']))
				{
					$_SESSION['perr'] = "File already uploaded. Please do not updated with same name.";
				}
				move_uploaded_file($_FILES['slikapk']['tmp_name'],"../upload_image/".$_FILES['slikapk']['name']);
				$KATEGORIJA_SLIKA = "upload_image/".$_FILES['slikapk']['name'];

				$glavnakatID = $_POST['glavna'];
				$podkat=$_POST['podkat'];

				$query="insert into podkat(glavnakat_id, podkat_naziv, podkat_slika) values('$glavnakatID','$podkat', '$KATEGORIJA_SLIKA')";
				$conn->exec($query);

				$_SESSION['ps'] = "You have successfully added a new subcategory.";
