...
<body onload="myFunction()" style="margin:0;">
	...
	<div class="container text-center" style="padding-bottom: 70px;">
		...
		<tbody>
			<?php
				...
				$q="select * from reklamacije where reklamacija_id='$_GET[id]'";
				$stmt = $conn->prepare($q);
				$stmt->execute();
				...
				while($row = $stmt->fetch())
				{
					...
					echo "<td> ".$row['opis_reklamacije'];
	 				echo '<td><center><input class="btn btn-success" name="tbn" type="submit" value="Change status">';
					echo "</tr>";
					echo '</form>';
				}
			?>
	...
	{
			$status = $_POST['status'];
			$reklamacijaID = $_GET['id'];

			include("../includes/connection.php");

			$UR = "update reklamacije set status_reklamacije='$status' where reklamacija_id='$reklamacijaID'";
			$stmt = $conn->prepare($UR);
			$stmt->execute();
			$_SESSION['ps'] = "You have successfully edited the status of the reclamation.";
	...
	?>
	<!--FOOTER-->
	<section class="copyright border bg-dark fixed-bottom">
		<div class="container">
				<div class="row">
