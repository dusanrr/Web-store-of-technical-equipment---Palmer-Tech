...
<body onload="myFunction()" style="margin:0;">
	...
	<tbody>
		...
		$totalEmpSQL = "select count(*) as broj FROM narudzbine where korisnik_id='$_SESSION[id_korisnika]'";
		...
		$empSQL = "select * FROM narudzbine N inner join podkat P where N.kategorija_id=P.glavnakat_id and N.korisnik_id='$_SESSION[id_korisnika]' LIMIT $startFrom, $showRecordPerPage";
		...
		while($row = $stmt->fetch())
		{
			...
			echo "<td> ".$row['kolicina'];
			echo "<td> ".$row['artikal_cena'];
			echo "<td> ".($row['kolicina']*$row['artikal_cena']);
			echo "<td> ".$row['datum_narudzbine'];
			echo "</tr>";
		}
		$npoklijentu="select * from narudzbine where korisnik_imeprezime='$_SESSION[imeprezime]'";
		$stmt = $conn->prepare($npoklijentu);
		$stmt->execute();
		$red = $stmt->fetch();

		$korisnikID = $red['korisnik_id'];
		$ime = $red['korisnik_imeprezime'];
		$lel = "select SUM(artikal_zaplacanje) as SUMA from narudzbine where korisnik_imeprezime like '$ime'";
	...
	<tr>
		...
		<td> <h4 class="text-success"><?php echo $glup['SUMA']; ?> RSD</h4></td>
	</tr>
	</tbody>
	</table>
	<!-- PAGINATION -->
