...
<div class="container">
	...
	<br><br><br>
	<div class="card-deck center-block justify-content-center" data-aos="fade-right"
 data-aos-offset="300"
 data-aos-easing="ease-in-sine">
		...
		<?php
			...
			$id=$_GET['id'];
			$q="select * from artikli where artikal_id=$id";
			$stmt = $conn->prepare($q);
			$stmt->execute();

			$row = $stmt->fetch();

			$naprodaju = "select * from artikli where artikal_id='$_GET[id]'";
			$stmt = $conn->prepare($naprodaju);
			$stmt->execute();
			$roww = $stmt->fetch();
			$lala = $roww['artikal_prodaja'];

			$query = "select * from podkat where podkat_id=".$row['artikal_podkat']."";
			...
			if($row != null)
			{
					...
					echo "
					<div class=\"card my-3 text-center justify-content-center\" style=\"max-width:400px;\" align=\"center\">
							<div class=\"card-header bg-dark text-white\">
								".$podkat['podkat_naziv']."
							</div>
							<img class=\"card-img-top lele\" src=\"$putanja_do_slike\" style=\"height:250px;\" alt=\"Card image\" style=\"width:100%\">
							<div class=\"card-body\">
								<h4 class=\"card-title\" style=\"font-size:12px;\">NAME: ".$row['artikal_naziv']."</h4>
								<h4 class=\"card-title\" style=\"font-size:12px;\">IN STOCK: ".$row['artikal_prodaja']."</h4>
								<h4 class=\"card-title\" style=\"font-size:12px; color:green;\">PRICE: ".$row['artikal_cena']." RSD</h4>
								<h4 class=\"card-title\" style=\"font-size:12px; color:red;\">WARRANTY: ".$row['artikal_garancija']." months</h4><br><br>
								<h4 class=\"card-title\" style=\"font-size:14px; color:grey;\">DESCRIPTION</h4>
								<p class=\"card-text\">".$row['artikal_opis']."</p>
							</div>
							...
							if($lala > 0)
			 				{
								if(isset($_SESSION['status']))
								{
									echo '<a class="btn btn-primary" href="dodavanje_u_korpu.php?aid='.$row['artikal_id'].'&aime='.$row['artikal_naziv'].'&apodkategorija='.$row['artikal_podkat'].'&acena='.$row['artikal_cena'].'">ADD TO CART</a>';
								}
								else
								{
									echo '<p class="btn btn-danger">You are not logged in!</p>';
								}
							}
							else
							{
								if(isset($_SESSION['status']))
								{
										echo '<a class="btn btn-primary">Out of stock</a>';
								}
								else
								{
										echo '<p class="btn btn-danger">You are not logged in!</p>';
