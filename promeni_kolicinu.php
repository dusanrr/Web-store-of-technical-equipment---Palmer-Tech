			...
			<?php
				...
				else
				{
							...
							<div class="navbar brand">
								<form class="form-inline" method="GET" action="pretrazivanje.php">
									<input class="form-control mr-sm-2" type="search" placeholder="Search" requiredrequired aria-label="Search" name="s">
									<button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
								</form>
							</div>';
				}
...
<table class="table table-hover text-center">
  ...
  <tbody>
    ...
    $query= "select * from korpa where artikal_id='$_GET[aid]' and korisnik_id='$_SESSION[id_korisnika]'";
    $stmt = $conn->prepare($query);
    $stmt->execute();

    while($row = $stmt->fetch())
    {
        $query2="select podkat_naziv from podkat where podkat_id='$row[kategorija_id]'";
        ...
        echo '<tr>';
        echo '<td><center> '.$i.'';
        echo '<td><center> '.$row1['podkat_naziv'].'';
        echo '<td><center> '.$row['artikal_naziv'].'';
        echo '<td><center> <input type="text" class="text-center" name="quan" required value="'.$row['kolicina'].'">';
        echo '<td><center> '.$row['artikal_cena'].' RSD';
        echo '<td><center> '.($row['kolicina']*$row['artikal_cena']).' RSD';
        echo '<td><center><input class="btn btn-success" name="dugme" type="submit" value="Change quantity">';
        echo '</tr>';
        echo '</form>';

        $tot = $tot + ($row['kolicina']*$row['artikal_cena']);
        $i++;
      }
    ?>
		<tr>
			...
			<td> <h4 class="text-success"><?php echo $tot; ?> RSD</h4></td>
		</tr>
  </tbody>
</table>
  ...
  if(isset($_POST['dugme']))
  {
    ...
    if($lala >= $ljelj)
    {
      $blaa = "update korpa set kolicina='$ljelj' where artikal_id='$artikalID'";
      $stmt = $conn->prepare($blaa);
      $stmt->execute();
			$_SESSION['ps'] = "You are successfully updated quantity of a product.";
      header("Location: korpa.php");
    }
    else
    {
			$_SESSION['perr'] = "There is not enought products in stock. Total in stock: $lala.";
			header("Location: korpa.php");
    }
  }
  ?>
