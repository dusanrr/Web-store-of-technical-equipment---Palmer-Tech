...
<html lang="en">
	...
	<body onload="myFunction()" style="margin:0;">
		...
		<div class="container text-center" style="padding-bottom: 70px;">
			<h4 class="animated bounceInRight" style="padding-bottom: 40px;">RECLAMATION</h4>
		<form method='POST' enctype="multipart/form-data" name="reklamacija">
		  <div class="form-group">
		    <label for="exampleFormControlSelect1">Product for reclamation</label><br>
				<select class="styled" required name="svemojeporudzbine">
					<?php

					include("includes/connection.php");

					$query="select * from narudzbine where korisnik_id='$_SESSION[id_korisnika]'";
					$stmt = $conn->prepare($query);
					$stmt->execute();

					while($row=$stmt->fetch())
					{
						echo "<option>".$row['artikal_naziv'];
					}
		  ...
		  <div class="form-group">
		    <label for="exampleFormControlTextarea1">Description of the problem</label>
		    <textarea class="form-control" required name='reklamacija' rows="5" style="resize:none; width:500px; margin:0 auto;"></textarea>
		  </div>
	...
	if(isset($_POST['btn']))
	{
		$ARTIKAL_ZA_REKLAMACIJU=$_POST['svemojeporudzbine'];
		$TEKST_REKLAMACIJE=$_POST['reklamacija'];
		$KID = $_SESSION['id_korisnika'];
		$IME = $_SESSION['imeprezime'];

		include("includes/connection.php");

		$query1="insert into reklamacije(korisnik_id, reklamirao,proizvod_za_reklamaciju, status_reklamacije,opis_reklamacije)
		values('$KID','$IME','$ARTIKAL_ZA_REKLAMACIJU', 'Pending','$TEKST_REKLAMACIJE')";

		$stmt = $conn->prepare($query1);
		$stmt->execute();

		$_SESSION['ps'] = "Reclamation successfully sent.";
		header("Location: moje_reklamacije.php");
	}
	?>
		</div>
		<!--SIGN UP-->
		...
		if(isset($_POST['register']))
		{
			...
			/*echo '<script>
				Alert.render("Uspesno ste se registrovali.");
				</script>';*/
